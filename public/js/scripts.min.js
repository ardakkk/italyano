...
$(document).ready(function () {
	...
	$('.slider-pizza').slick({
		...
		slidesToScroll: 3,
		adaptiveHeight: true,
		prevArrow: $('#choose-pizza .prev'),
		nextArrow: $('#choose-pizza .next'),
		responsive: [
			{
				breakpoint: 480,
				settings: {
	...
	//Form phone number
	$('#order-phone').mask('+7(000) 000-0000');
	//Form tab data
	$('#date-of-order').mask('00/00/0000');
	//Form tab time 
	$('#time-to-order').mask('00:00:00');
	//phone number on register popup
	$('#signupPhone').mask('+7(000) 000-0000');
	//login
	$('#loginPhone').mask('+7(000) 000-0000');
	//Slider Snacks
	$('.snacks-slider').slick({
		dots: true,
		arrows: true,
		...
		slidesToScroll: 4,
		adaptiveHeight: true,
		prevArrow: $('#snacks .prev'),
		nextArrow: $('#snacks .next'),
		responsive: [
			{
				breakpoint: 480,
				settings: {
	...
	$('.beverages-slider').slick({
		...
		slidesToShow: 3,
		adaptiveHeight: true,
		prevArrow: $('#beverages .prev'),
		nextArrow: $('#beverages .next'),
		responsive: [
			{
				breakpoint: 480,
				settings: {
	...
	//buttons choose-pizza 26 32
	$('.slider-pizza__inner .buttons button').click(function () {
	...
	//button count of order
	var count_btn = document.querySelectorAll('.choice__btn');
	count_btn.forEach(function (btn) {
		btn.addEventListener('click', function () {
			btn.style.display = 'none';
			var order = btn.nextElementSibling;
			order.style.display = 'flex'
		});
	});
	//hide btn
	function hideCountBtn(btn) {
	...
	function totalPrice(){
		...
		Array.from(totalPrice).forEach(function (tprice) {
			var total = 0;
			if (cart.length !== 0) {
				for (var i = 0; i < cart.length; i++) {
					total += parseInt(cart[i].total);
					tprice.textContent = total + ' тг';
	...
	function newItem(item){
		var tab = Curl === CONST_URL ? document.getElementsByClassName('tab-inner__orders')[0].getElementsByTagName('ul')[0] : null;
		var delete_btn = document.createElement('button');
		delete_btn.className = 'removeItem';
			delete_btn.textContent = '✖';
			var span = document.createElement('span');
			span.textContent = item.item + '....' + item.count + 'шт. / ' + item.price + 'тг';
			var li = document.createElement('li');
			li.dataset.type = item.type;
			li.appendChild(delete_btn);
			li.appendChild(span);
		Array.from(tab.getElementsByTagName('li')).forEach(function(l){
			console.log(item.item.slice(0,6) === l.textContent.slice(1,6));
			if(item.item.slice(0,3) === l.textContent.slice(1,4)){
				tab.removeChild(l);
				tab.appendChild(li);
			}else{
				tab.appendChild(li);
	...
	//count btn add items to localStroge
	function checkBtn() {
		var check = document.getElementsByClassName('count__submit');
		function replaceItem(oldItem, newItem) {
			var checkItem = oldItem !== null ? oldItem.filter(function (o) {
				return o.item === newItem.item;
			})[0].item === newItem.item : false;
			if (checkItem) {
				var replaceItem = oldItem.filter(function (o) {
					return o.item !== newItem.item;
				});
				replaceItem.push(newItem);
				localStorage.setItem(newItem.type, JSON.stringify(replaceItem));
				alertify.success(newItem.item + ' добавлено в корзину');
			} else {
				localStorage.setItem(newItem.type, JSON.stringify(newItem));
				alertify.success(newItem.item + ' добавлено в корзину');
			}
			return;
		}
		Array.from(check).forEach(function (btn) {
			btn.addEventListener('click', function (e) {
				if (e.target.className === 'count__submit pizza-btn' && btn.parentElement.parentElement.previousElementSibling.getElementsByClassName('active-btn')[0]) {
					var pizza = {};
					var newPizza = JSON.parse(localStorage.getItem('pizza')) || [];
					pizza.price = btn.parentElement.parentElement.previousElementSibling.getElementsByClassName('price')[0].dataset.price;
					pizza.item = btn.parentElement.previousElementSibling.previousElementSibling.previousElementSibling.previousElementSibling.innerText;
					pizza.sizeofPizza = btn.parentElement.parentElement.previousElementSibling.getElementsByClassName('active-btn')[0].innerText;
					pizza.count = btn.previousElementSibling.value;
					pizza.total = parseInt(pizza.price) * parseInt(pizza.count);
					pizza.type = 'pizza';
					newPizza.push(pizza);
					replaceItem(newPizza, pizza);
					hideCountBtn(btn);
					totalPrice();
					newItem(pizza);
				} else if (e.target.className === 'count__submit salad-btn') {
					var newSalad = JSON.parse(localStorage.getItem('salad')) || [];
					var salad = {};
					salad.price = btn.parentElement.parentElement.getElementsByClassName('price')[0].dataset.price;
					salad.item = btn.parentElement.parentElement.getElementsByTagName('h4')[0].textContent;
					salad.weight = btn.parentElement.parentElement.getElementsByClassName('weight')[0].textContent
					salad.count = btn.previousElementSibling.value;
					salad.total = parseInt(salad.price) * parseInt(salad.count);
					salad.type = 'salad';
					newSalad.push(salad);
					replaceItem(newSalad, salad);
					hideCountBtn(btn);
					totalPrice();
					newItem(salad);
				} else if (e.target.className === 'count__submit bvg-btn') {
					var bvg = {};
					var newBvg = JSON.parse(localStorage.getItem('bvg')) || [];
					bvg.price = btn.parentElement.parentElement.getElementsByClassName('price')[0].dataset.price;
					bvg.item = btn.parentElement.parentElement.getElementsByTagName('h4')[0].textContent;
					bvg.count = btn.previousElementSibling.value;
					bvg.total = parseInt(bvg.price) * parseInt(bvg.count);
					bvg.type = 'bvg';
					newBvg.push(bvg);
					replaceItem(newBvg, bvg);
					hideCountBtn(btn);
					totalPrice();
					newItem(bvg);
				} else {
					alertify.warning('Выберите размер Пиццы!');
				}
			});
		});
	}
	...
	function Cart(){
		var order = [];
		var pizza = JSON.parse(localStorage.getItem('pizza'));
		var salad = JSON.parse(localStorage.getItem('salad'));
		var bvg = JSON.parse(localStorage.getItem('bvg'));
		if (pizza !== null) {
			pizza.forEach(function (p) {
				order.push(p);
			});
		}
		if (salad !== null) {
			salad.forEach(function (s) {
				order.push(s);
			});
		}
		if (bvg !== null) {
			bvg.forEach(function (b) {
				order.push(b);
			});
		}
		return order;
	...
	if (Cart().length !== 0) {
		...
		var tab = Curl === CONST_URL ? document.getElementsByClassName('tab-inner__orders')[0].getElementsByTagName('ul')[0] : null;
		Curl === CONST_URL ? items.forEach(function (item) {
			var delete_btn = document.createElement('button');
			delete_btn.className = 'removeItem';
			delete_btn.textContent = '✖';
			var span = document.createElement('span');
		...
		var tab = document.getElementsByClassName('tab-inner__orders')[0].getElementsByTagName('ul')[0];
		var li = document.createElement('li');
		li.textContent = 'Не выбрано!'
		li.style.textAlign = 'center';
		li.style.fontFamily = 'Raleway-Bold';
		li.style.fontSize = '18px';
		tab.appendChild(li);
	}
	// remove Items in tab
	var removeBtn = document.getElementsByClassName('removeItem');
	Array.prototype.forEach.call(removeBtn, function (btn) {
		btn.addEventListener('click', function () {
			btn.parentElement.parentElement.removeChild(btn.parentElement);
			// console.log(btn.parentElement.dataset.type);
			if (btn.parentElement.dataset.type === 'pizza') {
				var data = JSON.parse(localStorage.getItem('pizza'));
				var value = btn.parentElement.getElementsByTagName('span')[0].textContent.slice(0, 5);
				var newData = data.filter(function (el) {
					return el.item.slice(0, 5) !== value;
				});
				if (newData.length === 0) {
					...
					localStorage.setItem('pizza', JSON.stringify(newData));
					totalPrice();
					// location.reload();
				}
			} else if (btn.parentElement.dataset.type === 'salad') {
				var data = JSON.parse(localStorage.getItem('salad'));
				var value = btn.parentElement.getElementsByTagName('span')[0].textContent.slice(0, 5);
				var newData = data.filter(function (el) {
					return el.item.slice(0, 5) !== value;
				});
				if (newData.length === 0) {
					...
					localStorage.setItem('salad', JSON.stringify(newData));
					totalPrice();
					// location.reload();
				}
			} else {
				var data = JSON.parse(localStorage.getItem('bvg'));
				var value = btn.parentElement.getElementsByTagName('span')[0].textContent.slice(0, 5);
				var newData = data.filter(function (el) {
					return el.item.slice(0, 5) !== value;
				});
				if (newData.length === 0) {
					localStorage.removeItem('bvg');
					totalPrice();
					// location.reload();
				} else {
					localStorage.setItem('bvg', JSON.stringify(newData));
					totalPrice();
					// location.reload();
				}
			}
			return alertify.error('Удалено из корзины');
		});
	});
	...
	//Append desktop nav to mobile nav
	if (navigator.userAgent.match(/Android/i) || navigator.userAgent.match(/webOS/i)
		...
		//onscroll action 
		$('.mobile-logo').append($('.top-header__logo img'));
		$('.under-menu-nav nav').append($('.top-header__nav ul'));
	}
	//burger button 
	$('.hamburger').click(function () {
		$(this).toggleClass('is-active');
		$('.under-menu-nav').toggleClass('show-mobile-nav');
	});
	$('p.btn-pc').click(function () {
		if (!$('.personal-profile').hasClass('active-login')) {
			$('.personal-profile').addClass('active-login');
		}
	});
	$('i.move-right').click(function () {
		$('.personal-profile').removeClass('active-login');
	});
	...
	function CurrentDay() {
		...
		today = dd + '/' + mm + '/' + yyyy;
		var dateOfOrder = document.getElementById('date-of-order');
		dateOfOrder.setAttribute('placeholder', today);
		dateOfOrder.setAttribute('value', today);
		//current time 
		var currentime = new Date();
		var datetime = currentime.getHours() + ':' + currentime.getMinutes() + ':' + currentime.getSeconds();
		var timeToOrder = document.getElementById('time-to-order');
		timeToOrder.setAttribute('placeholder', datetime);
		timeToOrder.setAttribute('value', datetime);
	};
	Curl === CONST_URL ? CurrentDay() : null;
	//checkout 
	$('.addToCart').click(function () {
		...
		dateToOrder = document.getElementById('date-of-order');
		timeToOrder = document.getElementById('time-to-order');
		comments = document.getElementById('comments');
		totalPrice = document.getElementsByClassName('total-price')[0].textContent;
		if (name.value.length !== 0 && phone.value.length !== 0 && address.value.length !== 0) {
			var data = {
				name: name.value,
				phone: phone.value,
				address: address.value,
				dateToOrder: dateToOrder.value,
				timeToOrder: timeToOrder.value,
			...
			alertify.error('Обязательно заполните это поле!');
	...
	//Tab checkout button to valid
	$('#checkout').click(function(){
		var inst = $('[data-remodal-id=checkout]').remodal();
		var name = document.getElementById('order-name'),
			phone = document.getElementById('order-phone'),
			address = document.getElementById('order-address');
			comments = document.getElementById('comments');
			...
			cart = Cart();
		if(sessionStorage.getItem('AUTH_STATUS') === 'false'){
			if(name.value.length !== 0 && phone.value.length !== 0 && address.value.length !== 0){
				if(cart.length === 0){
					alertify.error('В корзине пусто!')
				}else{
					inst.open();
				}
			}else{
			...
			if(comments.value.length !== 0){
				$('[data-remodal-id=auth-order]').remodal().open();
			}else{
				$('[data-remodal-id=auth-order]').remodal().close();
				alertify.error('Обязательно заполните это поле!');
				comments.style.borderColor = 'red';
			}
		}
	...
	$('.cabinet-btn').click(function(){
		var inst = $('[data-remodal-id=checkout]').remodal();
		localStorage.setItem('OrderAuthStatus',true);
		if(localStorage.getItem('OrderAuthStatus') ==='true'){
			inst.close();
			if (!$('.personal-profile').hasClass('active-login')) {
				$('.personal-profile').addClass('active-login');
			}
	...
	signin.click(function(){
		var login = document.getElementById('loginPhone').value;
		var pwd = document.getElementById('password').value;
		$.ajax({
			type: 'POST',
			url: '/user/signin',
			data: { phone: login, password: pwd, _token: token },
			dataType: 'JSON',
			success: function (res) {
	...
	//check auth order status
if(localStorage.getItem('OrderAuthStatus') === 'true'){
	$('[data-remodal-id=auth-order]').remodal().open();
}
sessionStorage.getItem('AUTH_STATUS') !== 'false' ? document.getElementsByClassName('auth-order__total')[0].innerHTML = '<b>Итого: </b>' + localStorage.getItem('totalPrice') + 'тг':null;
//user Order
$('#user-order').click(function(){
	var cart = Cart();
	var totalPrice = localStorage.getItem('totalPrice');
	var comments = document.getElementById('comments').value;
	var user = document.getElementsByClassName('auth-order__total')[0].dataset.user;
	var dateOfOrder = document.getElementById('date-of-order').value;
	var timeToOrder = document.getElementById('time-to-order').value;
	
	if(cart.length !== 0 && comments !== 0 && totalPrice !== '0'){
		var data = {
			cart: cart,
			totalPrice: totalPrice,
			comments: comments.length !==0 ? comments:'Good Service',
			date: dateOfOrder,
			time: timeToOrder,
			userOrder: user,
			_token: token
...
function checkScroll() {
	var nav = $('.top-header').first();
	//   console.log($(document).scrollTop());
	if ($(document).scrollTop() > 0 && !nav.hasClass('fixed-top')) {
		$('.header-body').css('padding-top', '100px');
		nav.addClass('fixed-top');
	} else if ($(document).scrollTop() === 0 && nav.hasClass('fixed-top')) {
		$('.header-body').css('padding-top', '0px');
...
if(logout){
	logout.addEventListener('click',function(){
		$.get('/user/logout',function(res){
			...
			localStorage.setItem('OrderAuthStatus',false);
			location.reload();
		});
	});
}
//Signup 
$('#resignupPassword').keyup(function(e){
	if($('#signupPassword').val() !== e.target.value){
...
$('#signupBtn').click(function(){
	...
	var signupAddress = $('#signupAddress').val();
	var signupPwd = document.getElementById('signupPassword').value;
	var rePwd = $('#resignupPwd').val();
		if(signupName.length !== 0 && signupEmail.length !== 0 && signupPhone.length !== 0 && signupPwd.length !== 0){
		var data = {
			name: signupName,
			email: signupEmail,
			phone: signupPhone,
		...
		var input = document.getElementById('formSignup').getElementsByClassName('signupInput');
...
$('#editBtn').click(function(){
	var inst = $('[data-remodal-id=edit]').remodal();
	...
	if(editName.val().length !== 0 && editEmail.val() !== 0 && editPhone.val() !== 0 && editAddress.val() !== 0){
		var data = {
			name: editName.val(),
			email: editEmail.val(),
			phone: editPhone.val(),
...
//mobile profile
document.getElementsByClassName('personal-cabinet')[0].addEventListener('click',function(){
	document.getElementsByClassName('under-menu-nav')[0].classList.remove('show-mobile-nav');
	document.getElementsByClassName('personal-profile')[0].classList.add('active-login');
});
